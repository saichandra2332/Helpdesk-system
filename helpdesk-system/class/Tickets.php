...
class Tickets extends Database {  
	...
	public function showTickets(){
		...
		if(isset($_SESSION["role"]) && $_SESSION["role"] === 'technical') {
			...
			$sqlWhere .= " FIND_IN_SET('$techId', t.technical_team)   AND t.technical = 'Y' ";
		}
		$time = new time;  			 
		$sqlQuery = "SELECT t.id, t.uniqid, t.title, t.init_msg as message, t.date, t.technical, t.last_reply, t.resolved, u.name as creater, d.name as department, u.user_type, t.user, t.user_read, t.admin_read
			FROM hd_tickets t 
		...
		while( $ticket = mysqli_fetch_assoc($result) ) {		
			...
			$color ='';
			if($ticket['technical'] == 'Y')	{
				$color = 'btn-success';
			} else if($ticket['technical'] == 'N') {
				$color = 'btn-danger';
			}	
			$disbaled = '';
		if (!isset($_SESSION["admin"]) && (!isset($_SESSION["role"]) || $_SESSION["role"] !== 'technical')) {
			$disabled = 'disabled';
		}
			...
			if (isset($_SESSION["admin"]) || (isset($_SESSION["role"]) && $_SESSION["role"] === 'technical')) {
				...
				$ticketRows[] = ''; // Empty for other users
			}
			
			if (isset($_SESSION["admin"])) {
				$ticketRows[] = '<button type="button" name="send" id="'.$ticket["id"].'" class="btn '.$color.' btn-xs sendtech">Send</button>';
			} else {
				$ticketRows[] = ''; // Empty value for non-admin users
			}
	...
	public function fetchUsers(){
		$query = "SELECT id, name as name_ FROM hd_users where user_type = 'technical'"; 
	...
	public function sendTicketDetails() {
		if ($_POST['ticketId']) {    
			// Update the 'technical' column to 'Y' for the given ticket ID
			$updateQuery = "
				UPDATE ".$this->ticketTable." 
				SET technical = 'Y' ,technical_team = '".$_POST["userIds"]."'
				WHERE id = '".$_POST["ticketId"]."'";
